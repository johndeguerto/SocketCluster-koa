...
var options = {
  workers: config.CPUCOUNT || Number(process.env.SOCKETCLUSTER_WORKERS) || 1,
  brokers: config.CPUCOUNT || Number(process.env.SOCKETCLUSTER_BROKERS) || 1,
  port: config.PORT || Number(process.env.SOCKETCLUSTER_PORT) || 8000,
  // You can switch to 'sc-uws' for improved performance.
  wsEngine: process.env.SOCKETCLUSTER_WS_ENGINE || 'ws',
  appName: config.APPNAME || process.env.SOCKETCLUSTER_APP_NAME || null,
